# Leetcode Qn: 756. Pyramid Transition Matrix 
...
# The patterns are given as a list of three-letter strings allowed, where the first two characters of a 
# pattern represent the left and right bottom blocks respectively, and the third character is the top block.
# For example, "ABC" represents a triangular pattern with a 'C' block stacked on top of an 'A' (left) and 'B' (right) block. Note that this is 
# different from "BAC" where 'B' is on the left bottom and 'A' is on the right bottom.
        ...
        def can_build(row: str) -> bool:
            ...
            def dfs_next(i, cur, results):
                ...
                for top in mapping[pair]:
                    dfs_next(i+1, cur+top, results)

            next_rows = []
            dfs_next(0, "", next_rows)
