# Codeforces Qn name: A. Yes or Yes
...
# Unfortunately, Fernando does not want you to combine "Yes OR Yes", as he has experienced
# trauma relating to a certain song.

# Determine whether it is possible to reduce s
#  to a single character by repeatedly applying the operation above, without ever
#  combining two Y
# 's.
...
# Output
# For each test case, print "YES" if the string can be reduced to a single character by 
# repeatedly applying the described operation, and "NO" otherwise.
...
#  or YYYNY–––→YYYY
# . However, in either case, it is not possible to perform any more operations 
# without combining two Y's. Thus, the answer is NO.

# In the fifth test case, the following is a valid sequence of operations:
#     NNN–––NN→NN–––NN→NNN–––→NN–––→N. Thus, the answer is YES.
