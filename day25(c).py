# Leetcode Qn: 3562. Maximum Profit from Trading Stocks with Discounts
# You are given an integer n, representing the number of employees in a company. Each employee is assigned a unique ID 
# from 1 to n, and employee 1 is the CEO, is the direct or indirect boss of every employee. You are given two 1-based integer arrays,
# present and future, each of length n, where: present[i] represents the current price at which the ith employee can buy a stock today.
...
class Solution:
    def maxProfit(self, n, present, future, hierarchy, budget):
        ...
        @lru_cache(None)
        def dfs(u, bossBought):
            cost_u = (present[u-1]//2) if bossBought else present[u-1]
            ...
            for v in tree[u]:
                child = dfs(v, False)
                dp_not = merge(dp_not, list(child))
            ...
            for v in tree[u]:
                child = dfs(v, True)
                dp_buy = merge(dp_buy, list(child))

            res = [NEG]*(budget+1)
            for i in range(budget+1):
                res[i] = max(dp_not[i], dp_buy[i])
            return tuple(res)

        ans = dfs(1, False)
        return max(x for x in ans if x != NEG)